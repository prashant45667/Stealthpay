  ...
  return (
    <div className="luxury-card rounded-3xl p-6 sm:p-8 space-y-6 relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/5">
        ...
        {/* View mode switcher */}
        <div className="flex items-center space-x-1 p-1 rounded-full bg-onyx-950/80 border border-white/5 text-xs font-mono">
          <button
            onClick={() => setViewMode('split')}
            className={`px-3.5 py-1.5 rounded-full font-medium transition-all ${
              viewMode === 'split' ? 'bg-white text-onyx-950 font-bold shadow-sm' : 'text-onyx-400 hover:text-white'
            }`}
          >
            Side-by-Side View
          </button>
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full font-medium transition-all ${
              viewMode === 'public' ? 'bg-cyan-neon text-onyx-950 font-bold shadow-sm' : 'text-onyx-400 hover:text-white'
            }`}
          >
            Public Ledger Only
          </button>
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full font-medium transition-all ${
              viewMode === 'private' ? 'bg-emerald-neon text-onyx-950 font-bold shadow-sm' : 'text-onyx-400 hover:text-white'
            }`}
          >
      ...
      <div className={`grid gap-6 ${viewMode === 'split' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
        ...
        {(viewMode === 'split' || viewMode === 'public') && (
          <div className="rounded-2xl bg-onyx-950/90 border border-cyan-neon/30 p-5 sm:p-6 space-y-4 relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              <div className="p-3 rounded-xl bg-onyx-900 border border-white/5">
            ...
            <div className="p-3 rounded-xl bg-onyx-900 border border-white/5 space-y-1 font-mono text-xs">
              <div className="text-[10px] text-onyx-400">ZK Merkle Commitment Root</div>
              <div className="text-[11px] text-onyx-300 truncate">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {recipients.map((_, i) => (
                  <div
                    key={i}
                    className="p-2.5 rounded-xl bg-onyx-900/80 border border-white/5 flex items-center justify-between text-xs font-mono"
                  >
                    <div className="flex items-center space-x-2">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-neon/10 text-emerald-neon">
                      🔒 Shielded Amount
                    </span>
                  </div>
                ))}
        ...
        {(viewMode === 'split' || viewMode === 'private') && (
          <div className="rounded-2xl bg-onyx-950/90 border border-emerald-neon/30 p-5 sm:p-6 space-y-4 relative overflow-hidden">
            <div className="flex items-center justify-between pb-3 border-b border-white/5">
              <div className="flex items-center space-x-2">
                <ShieldCheck className="w-4 h-4 text-emerald-neon" />
                <span className="text-xs font-bold uppercase tracking-wider text-emerald-neon font-mono">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              ...
              <div className="p-3 rounded-xl bg-onyx-900 border border-white/5">
                ...
                <div className="text-base font-bold text-emerald-neon mt-0.5" suppressHydrationWarning>
                  {formatNumber(poolAmount)} tDUST
                </div>
              </div>
            </div>
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {recipients.map((r, i) => {
                  ...
                  return (
                    <div
                      key={r.id || i}
                      className="p-2.5 rounded-xl bg-onyx-900/80 border border-white/5 flex items-center justify-between text-xs"
                    >
                      <div className="flex items-center space-x-2">
                        <div className="w-5 h-5 rounded-full bg-onyx-800 border border-white/10 flex items-center justify-center text-[9px] text-onyx-300 font-bold">
                          {r.name.charAt(0)}
                        </div>
                        <div>
                      ...
                      <div className="text-right font-mono">
                        <div className="text-emerald-neon font-bold text-xs" suppressHydrationWarning>{formatNumber(r.amount)} tDUST</div>
