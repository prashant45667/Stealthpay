...
export const DisbursementStudio: React.FC<DisbursementStudioProps> = ({
  ...
  const handleAddRecipient = () => {
    ...
    const newRecipient: RecipientRow = {
      ...
      notes: 'Private split allocation',
      salt: generateBytes32('salt'),
    };
  ...
  return (
    <div className="luxury-card rounded-3xl p-6 sm:p-8 space-y-6 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/5">
        <div>
          ...
          <p className="text-xs text-onyx-400 mt-1">
            Build confidential payroll batches. Individual payouts are isolated in Zero-Knowledge witnesses.
          </p>
        </div>

        {/* Preset Selector */}
        <div className="flex items-center space-x-1.5 overflow-x-auto p-1 rounded-full bg-onyx-950/80 border border-white/5">
          <span className="text-[10px] font-semibold text-onyx-400 uppercase tracking-wider px-3 font-mono">
            Presets:
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-onyx-950/70 border border-white/5 space-y-2">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-onyx-900 border border-white/10 text-white font-mono text-xl font-bold focus:outline-none focus:border-cyan-neon transition"
            />
            <button
              type="button"
              onClick={handleAutoBalancePool}
              className="absolute right-3 top-2.5 text-[11px] px-2.5 py-1 rounded-lg bg-onyx-800 hover:bg-onyx-700 text-onyx-300 hover:text-white transition font-mono"
        ...
        <div className="p-5 rounded-2xl bg-onyx-950/70 border border-white/5 space-y-2">
          ...
          <input
            ...
            className="w-full px-4 py-2.5 rounded-xl bg-onyx-900 border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-neon transition"
          />
        </div>
      ...
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center space-x-2">
            <span className="text-xs font-bold uppercase tracking-wider text-onyx-300 font-mono">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              type="button"
              className="inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-full bg-onyx-950 hover:bg-onyx-900 text-onyx-300 hover:text-cyan-neon text-xs font-mono font-medium border border-white/10 transition"
            ...
            <button
              ...
              className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-full bg-emerald-neon/15 hover:bg-emerald-neon/25 text-emerald-neon text-xs font-mono font-semibold border border-emerald-neon/30 transition disabled:opacity-40"
            >
        ...
        {/* Table Content */}
        <div className="overflow-x-auto rounded-2xl border border-white/5 bg-onyx-950/60">
          <table className="w-full text-left text-xs">
            <thead className="bg-onyx-900/90 text-onyx-400 uppercase font-mono tracking-wider border-b border-white/5">
              <tr>
            ...
            <tbody className="divide-y divide-white/5 font-mono">
              {recipients.map((recipient, idx) => {
                ...
                return (
                  <tr key={recipient.id} className="hover:bg-onyx-900/40 transition">
                    <td className="py-3.5 px-4 text-onyx-500 font-mono">{idx + 1}</td>
                    
                    {/* Name & Role */}
                    <td className="py-3.5 px-4">
                      <div className="flex items-center space-x-2">
                        <div className="w-6 h-6 rounded-full bg-onyx-800 border border-white/10 flex items-center justify-center text-[10px] text-onyx-300 font-bold">
                        ...
                        <input
                          ...
                          className="px-2.5 py-1 rounded-lg bg-onyx-900 border border-white/5 text-white font-sans text-xs focus:outline-none focus:border-emerald-neon"
                        />
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      <input
                        ...
                        className="w-48 sm:w-56 px-2.5 py-1 rounded-lg bg-onyx-900 border border-white/5 text-onyx-400 font-mono text-[11px] focus:outline-none focus:border-emerald-neon"
                      />
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      <input
                        ...
                        className="w-28 px-2.5 py-1 rounded-lg bg-onyx-900 border border-white/5 text-onyx-400 font-sans text-xs focus:outline-none focus:border-emerald-neon"
                      />
                    </td>
                    ...
                    <td className="py-3.5 px-4">
                      <input
                        ...
                        className="w-28 px-2.5 py-1 rounded-lg bg-onyx-900 border border-emerald-neon/30 text-emerald-neon font-mono font-bold focus:outline-none focus:border-emerald-neon"
                      />
                    </td>
      ...
      <div className="p-5 rounded-2xl bg-onyx-950/90 border border-white/5 space-y-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-onyx-400" suppressHydrationWarning>
                {isSolvent
                  ? 'Private witness sum strictly matches public pool. Zero-Knowledge proof guarantees 100% solvency.'
        ...
        <div className="pt-3 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-white/5">
          ...
          <button
            ...
            className={`flex items-center justify-center space-x-2.5 px-8 py-3 rounded-full font-bold font-mono text-xs transition ${
              canExecute
                ? 'bg-white text-onyx-950 shadow-pill hover:bg-neutral-200 cursor-pointer scale-100 hover:scale-[1.02]'
                : 'bg-onyx-800 text-onyx-500 border border-white/5 cursor-not-allowed'
            }`}
          >
